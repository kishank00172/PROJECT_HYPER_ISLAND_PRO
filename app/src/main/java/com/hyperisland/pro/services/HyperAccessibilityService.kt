...
class HyperAccessibilityService : AccessibilityService() {
    ...
    companion object {
        private const val FLAGS_VISUAL =
    ...
    private fun postToggleExpanded() = mainHandler.post { if (notificationMode && currentStage == IslandStage.STAGE3_FULL) openCurrentNotification() else postToggleExpandFromApp() }
    ...
    private fun setStageAnimated(targetStage: IslandStage, reason: ExpandReason) {
        ...
        val animator = ValueAnimator.ofFloat(0f, 1f).apply {
            ...
            addListener(onEnd = {
                if (targetStage == IslandStage.STAGE1_IDLE) { contentContainer?.alpha = 0f; contentContainer?.visibility = View.GONE; notificationMode = false; updateVisualRootStatic(visualRoot!!, targetW, targetH); updateTouchWindow(targetW, targetH) }
    ...
    fun updateAllToCurrentState() {
        ...
        updateOutlineForIsland(w, h, r); updateVisualRootStatic(visualRoot!!, w, h); updateTouchWindow(w, h)
        islandView?.layoutParams = islandLayoutParams; visualRoot?.invalidateOutline()
    }

    private fun getTargetWidth(s: IslandStage): Int = when(s) { 
    ...
    private fun getTargetHeight(s: IslandStage): Int = when(s) { 
    ...
    private fun getTargetRadius(s: IslandStage): Int = if (s == IslandStage.STAGE3_FULL) dp(AppSettings.getIslandExpandedCornerRadiusDp(this)) else dp(AppSettings.getIslandCornerRadiusDp(this))
    private fun buildDisplayText(appName: String, t: String, m: String): DisplayText = DisplayText(appName.trim().ifBlank { "App" }, t.trim().ifBlank { m.trim() }, if (t.trim().equals(appName.trim(), true)) "" else m.trim())
    ...
    private fun showIslandInternal() {
        ...
        contentContainer = LinearLayout(this).apply {
            ...
            titleText = TextView(context).apply { setTextColor(Color.WHITE); typeface = Typeface.DEFAULT_BOLD; maxLines = 1; ellipsize = TextUtils.TruncateAt.END }
            messageText = TextView(context).apply { setTextColor(Color.rgb(210, 210, 216)); ellipsize = TextUtils.TruncateAt.END }
        ...
        touchView = FrameLayout(this).apply { 
            isClickable = true
            setOnClickListener { if (notificationMode && currentStage == IslandStage.STAGE3_FULL) openCurrentNotification() else postToggleExpandFromApp() }
        }
    ...
    private fun postToggleExpandFromApp() = mainHandler.post { toggleExpandFromApp(this@HyperAccessibilityService) }
    ...
    private fun updateTouchWindow(widthPx: Int, heightPx: Int) { 
        ...
        tp.width = widthPx; tp.height = touchH; tp.x = dp(AppSettings.getIslandXDp(this)); tp.y = dp(AppSettings.getIslandYDp(this))
        try { windowManager?.updateViewLayout(tv, tp) } catch (_: Exception) {} 
    }
    ...
    private fun createVisualParams(): WindowManager.LayoutParams = WindowManager.LayoutParams(
        dp(AppSettings.getIslandExpandedWidthDp(this)), dp(AppSettings.getIslandExpandedHeightDp(this) + 40),
        WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, FLAGS_VISUAL, PixelFormat.TRANSLUCENT
    ...
    private fun createTouchParams(w: Int, h: Int): WindowManager.LayoutParams = WindowManager.LayoutParams(
        w, dp(25), WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, FLAGS_TOUCH, PixelFormat.TRANSLUCENT
    ...
    private fun lerpEven(s: Int, e: Int, p: Float): Int { val outVal = (s + ((e - s) * p)).roundToInt(); return if (outVal % 2 != 0) outVal + 1 else outVal }
