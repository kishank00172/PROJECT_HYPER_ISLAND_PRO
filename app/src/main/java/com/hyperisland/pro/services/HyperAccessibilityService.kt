...
class HyperAccessibilityService : AccessibilityService() {
    ...
    private fun playFluidTransitionAnimation(next: NotificationModel) {
        val exit = ValueAnimator.ofFloat(0f, 1f).apply { duration = 200L; interpolator = AccelerateInterpolator(); addUpdateListener { gridRoot?.alpha = 1f - it.animatedValue as Float; gridRoot?.translationY = it.animatedValue as Float * 20f } }
        val entry = ValueAnimator.ofFloat(0f, 1f).apply { duration = 250L; interpolator = DecelerateInterpolator(); addUpdateListener { gridRoot?.alpha = it.animatedValue as Float; gridRoot?.translationY = -20f * (1f - it.animatedValue as Float) } }
        exit.addListener(object : android.animation.Animator.AnimatorListener {
            override fun onAnimationStart(a: Animator) {}
            override fun onAnimationCancel(a: Animator) {}
            override fun onAnimationRepeat(a: Animator) {}
            override fun onAnimationEnd(a: Animator) { updateNotificationContent(next); entry.start() }
        })
    ...
    private fun setupActionTiles(pkg: String, customActions: List<String>?) {
        ...
        actions.forEach { action ->
            val btn = TextView(this).apply {
                text = action; setTextColor(Color.WHITE); textSize = 11f; gravity = Gravity.CENTER; setPadding(dp(10), 0, dp(10), 0)
                ...
                isClickable = true
                setOnClickListener { 
                    val oldText = text; text = "✓ $action"; setTextColor(Color.GREEN)
                    postDelayed({ if (action == "Dismiss") postCollapseIsland() else { text = oldText; setTextColor(Color.WHITE) } }, 600)
                }
            }
            val lp = LinearLayout.LayoutParams(dp(btnWidth), dp(28)); lp.marginStart = dp(6); footerActions?.addView(btn, lp)
        }
    }
    ...
    private fun triggerFluidExpansion() {
        ...
        val initialH = dp(AppSettings.getIslandHeightDp(this)); val targetH = dp(AppSettings.getIslandExpandedHeightDp(this))
        ...
        val ping = ValueAnimator.ofFloat(0f, 1f).apply { duration = 100L; addUpdateListener { updateIslandLayout(lerp(startW, pingW, it.animatedValue as Float), initialH, startR) } }
        val expand = ValueAnimator.ofFloat(0f, 1f).apply { duration = 350L; interpolator = fluidInterpolator; addUpdateListener { val t = it.animatedValue as Float; updateIslandLayout(lerp(pingW, targetW, t), lerp(initialH, targetH, t), lerp(startR, targetR, t)); gridRoot?.alpha = t; gridRoot?.visibility = View.VISIBLE } }
    ...
    private fun setStageAnimated(target: IslandStage, reason: ExpandReason) {
        ...
        val startR = islandBackground?.cornerRadius ?: dp(AppSettings.getIslandCornerRadiusDp(this)).toFloat()
        ...
        val anim = ValueAnimator.ofFloat(0f, 1f).apply {
            ...
            addUpdateListener { val t = it.animatedValue as Float; updateIslandLayout(lerp(startW, targetW, t), lerp(startH, targetH, t), lerp(startR, targetR, t)); if (target == IslandStage.STAGE1_IDLE) gridRoot?.alpha = 1f - t }
            addListener(object : android.animation.Animator.AnimatorListener {
                override fun onAnimationStart(a: Animator) {}
                override fun onAnimationEnd(a: Animator) { if (target == IslandStage.STAGE1_IDLE) { gridRoot?.visibility = View.GONE; notificationMode = false }; forceRegionUpdate() }
                override fun onAnimationCancel(a: Animator) {}
                override fun onAnimationRepeat(a: Animator) {}
            })
        }
    ...
    private fun getTargetRadius(s: IslandStage) = when(s) { IslandStage.STAGE3_FULL -> AppSettings.getIslandExpandedCornerRadiusDp(this); else -> AppSettings.getIslandCornerRadiusDp(this) }
    ...
    private fun scheduleAutoCollapse() {
        ...
        autoCollapseRunnable = Runnable { if (notificationQueue.isNotEmpty()) processNextInQueue() else setStageAnimated(IslandStage.STAGE1_IDLE, ExpandReason.AUTO_NOTIFICATION) }
    ...
    private fun showIslandInternal() {
        ...
        islandView = FrameLayout(this).apply {
            ...
            gridRoot = LinearLayout(context).apply {
                ...
                val contentSec = LinearLayout(context).apply {
                    ...
                    headerLine = TextView(context).apply { setTextColor(Color.rgb(0, 150, 255)); textSize = 10.5f; maxLines = 1; ellipsize = TextUtils.TruncateAt.END }
        ...
        visualRoot?.addView(islandView, islandLayoutParams); updateOutlineForIsland(dp(AppSettings.getIslandWidthDp(this)), dp(AppSettings.getIslandHeightDp(this)), dp(AppSettings.getIslandCornerRadiusDp(this)).toFloat())
    ...
    private fun updateAllToCurrentState() { updateIslandLayout(dp(getTargetWidth(currentStage)), dp(getTargetHeight(currentStage)), dp(getTargetRadius(currentStage)).toFloat()) }
