...
class HyperAccessibilityService : AccessibilityService() {
    ...
    companion object {
        // VISUAL LAYER: Always Match Parent but NEVER touchable
        ...
        // TOUCH LAYER: Dynamic size, handles clicks
    ...
    private fun setStageAnimated(targetStage: IslandStage, reason: ExpandReason) {
        ...
        val animator = ValueAnimator.ofFloat(0f, 1f).apply {
            ...
            addListener(onEnd = {
                if (targetStage == IslandStage.STAGE1_IDLE) { contentContainer?.alpha = 0f; contentContainer?.visibility = View.GONE; notificationMode = false; updateTouchWindow(targetW, targetH) }
    ...
    private fun updateVisualRootStatic(root: FrameLayout, w: Int, h: Int) { 
        visualParams?.apply { width = -1; height = h + dp(40); y = dp(AppSettings.getIslandYDp(this@HyperAccessibilityService)) }
    ...
    private fun createVisualParams() = WindowManager.LayoutParams(-1, dp(AppSettings.getIslandExpandedHeightDp(this) + 40), WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, FLAGS_VISUAL, PixelFormat.TRANSLUCENT).apply { gravity = Gravity.TOP or Gravity.CENTER_HORIZONTAL; y = dp(AppSettings.getIslandYDp(this@HyperAccessibilityService)); if (Build.VERSION.SDK_INT >= 28) layoutInDisplayCutoutMode = 1; title = "HyperIslandProVisual" }
    private fun createTouchParams(w: Int, h: Int) = WindowManager.LayoutParams(w, dp(25), WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, FLAGS_TOUCH, PixelFormat.TRANSLUCENT).apply { gravity = Gravity.TOP or Gravity.CENTER_HORIZONTAL; x = dp(AppSettings.getIslandXDp(this@HyperAccessibilityService)) ; y = dp(AppSettings.getIslandYDp(this@HyperAccessibilityService)); if (Build.VERSION.SDK_INT >= 28) layoutInDisplayCutoutMode = 1; title = "HyperIslandProTouch" }
