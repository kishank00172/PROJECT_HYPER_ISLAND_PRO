...
class HyperAccessibilityService : AccessibilityService() {
    ...
    private fun playFluidTransitionAnimation(next: NotificationModel) {
        ...
        exitAnimator.addListener(object : AnimatorListenerAdapter() {
            override fun onAnimationEnd(a: Animator) { updateNotificationContent(next); contentContainer?.translationY = -80f; contentContainer?.alpha = 0f; entryAnimator.start() }
        })
    ...
    private fun showIslandInternal() {
        ...
        // Touch Interceptor logic inside standard FrameLayout
        visualRoot = object : FrameLayout(this) {
            override fun dispatchTouchEvent(ev: MotionEvent): Boolean {
                val island = islandView ?: return false
                val pos = IntArray(2)
                island.getLocationOnScreen(pos)
                val rect = Rect(pos[0], pos[1], pos[0] + island.width, pos[1] + island.height)
                return if (rect.contains(ev.rawX.toInt(), ev.rawY.toInt())) super.dispatchTouchEvent(ev) else false
            }
