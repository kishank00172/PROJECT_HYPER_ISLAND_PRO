...
object AppSettings {
    ...
    private const val CURRENT_DEFAULTS_VERSION = 4
    ...
    fun ensurePhaseDefaults(context: Context) {
        ...
        if (version < CURRENT_DEFAULTS_VERSION) {
            ...
            if (version < 3) {
                editor.putInt(KEY_ISLAND_EXPANDED_WIDTH_DP, DEFAULT_ISLAND_EXPANDED_WIDTH_DP)
                    .putInt(KEY_ISLAND_EXPANDED_HEIGHT_DP, DEFAULT_ISLAND_EXPANDED_HEIGHT_DP)
            }
            if (version < 4) {
                editor.putInt(KEY_ISLAND_STAGE2_WIDTH_DP, DEFAULT_ISLAND_STAGE2_WIDTH_DP)
            }
